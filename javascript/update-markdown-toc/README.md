...
This is a safety measure to prevent accidental modification of third-party dependencies.

### Overriding the Default Exclusion List

By default `node_modules` is excluded from recursive traversal: when using `--recursive` the tool will skip any directory
  named `node_modules` and its contents. This prevents accidental processing or modification of third-party files.
You may override the default exclusion list by providing a **comma-separated list of directory names** via:

When `--exclude` is specified:

- The provided list completely replaces the default exclusion list.
- `node_modules` will **not** be excluded unless explicitly included in the list.

### Examples

Exclude both `node_modules` and `dist`:
...
The exclusion list matches directory names exactly (not glob patterns).

Matching is applied during recursive descent.

Exclusions apply only in --recursive mode.

In single-file mode, exclusions are irrelevant.
