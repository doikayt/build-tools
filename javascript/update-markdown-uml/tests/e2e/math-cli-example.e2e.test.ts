...
function npmPack(packageDir: string, dest: string): string {
  ...
  // would redundantly `rm -rf dist` + rebuild here, racing against sibling tasks (e.g.
  // update-markdown-toc:test) reading that same dist/ via the npm workspace symlink.
