...
`,
    ...
    test("exits 0 when --project-json is omitted", () => {
        const result = spawnSync("node", [BIN, markdownPath], { encoding: "utf-8" });
    ...
    test("update-markdown-uml stub still runs when --project-json is omitted", () => {
        const output = execSync(`node "${BIN}" "${markdownPath}"`, { encoding: "utf-8" });
...
`,
        ...
        const result = spawnSync("node", [BIN, "--project-json", projectPath, markdownPath], {
            encoding: "utf-8",
        });
