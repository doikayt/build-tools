...
export function extractLinks(
  ...
  const managedRanges = findManagedBlockRanges(
    markdownText,
    startMarker,
    endMarker
  );
  ...
  function processNode(href: string, line: number): void {
    if (isInManagedBlock(line, managedRanges)) {
      onDebug?.(
        `extractLinks: SKIPPED (managed block) href=${href} line=${line}`
      );
      skippedCount++;
      return;
    }
    ...
    if (kind === null) {
      onDebug?.(
        `extractLinks: SKIPPED (ignored scheme) href=${href} line=${line}`
      );
      skippedCount++;
      return;
    }
