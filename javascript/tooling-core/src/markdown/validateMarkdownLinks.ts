...
export async function validateMarkdownLinks(
  ...
  options.onDebug?.(
    `validateMarkdownLinks: parsed headingCount=${headings.length}`
  );
  ...
  for (const link of links) {
    if (link.kind === "fragment") {
      ...
      if (error !== null) {
        ...
        options.onDebug?.(
          `validateMarkdownLinks: fragment OK href=${link.href}`
        );
        validatedCount++;
      }
    }
    ...
    if (link.kind === "relative") {
      ...
      if (error !== null) {
        ...
        options.onDebug?.(
          `validateMarkdownLinks: relative OK href=${link.href}`
        );
        validatedCount++;
      }
    }
  ...
  const externalLinks =
    options.validateExternal !== false
      ? links.filter((l) => l.kind === "external")
      : [];

  options.onDebug?.(
    `validateMarkdownLinks: external links to validate: count=${
