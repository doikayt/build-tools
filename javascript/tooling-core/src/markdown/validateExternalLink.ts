...
export async function validateExternalLink(
  ...
  options.onDebug?.(
    `validateExternalLink: fetching href=${link.href} timeoutMs=${timeoutMs}`
  );
  ...
  try {
    ...
    options.onDebug?.(
      `validateExternalLink: fetch error href=${link.href} reason=${reason}`
    );
    return {
      file: filePath,
      line: link.line,
  ...
  if (status === 403) {
    return {
      ...
      link: link.href,
      reason:
        "access forbidden (403) — resource may exist but requests are blocked",
    } as LinkValidationWarning;
  }
