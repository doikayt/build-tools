...
export function validateConfig(config: RunConfig, _positionals: string[]): void {
...
export function parseStandardCli(
    ...
    const valueRequiringFlags = new Set<string>(
        (pluginOptions ?? []).filter(o => o.requiresValue === true).map(o => o.flag)
    );
...
export function parseBooleanOption(flag: string, map: Map<string, string | boolean>): boolean {
...
export function buildConfig<TConfig extends RunConfig>(
    ...
    return parseOptions ? parseOptions(standard, passthroughMap) : (standard as TConfig);
}
