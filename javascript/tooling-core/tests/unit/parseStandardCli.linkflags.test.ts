...
const linkPluginOptions: PluginOption[] = [
  { flag: "--no-external-link-check", description: "Skip external link validation" },
...
describe('parseStandardCli — link flag passthrough (Option B)', () => {
  ...
  test('--no-external-link-check passes through', () => {
    const result = parseStandardCli(['--no-external-link-check'], linkPluginOptions)
  ...
  test('--link-timeout-ms passes through with value', () => {
    const result = parseStandardCli(['--link-timeout-ms', '5000'], linkPluginOptions)
