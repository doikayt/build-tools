...
describe('runLinkValidation()', () => {
  ...
  test('sets exitCode 1 for broken fragment link', async () => {
    const file = writeFile('broken-fragment.md', '# Title\n\n[link](#nonexistent)\n')
  ...
  test('sets exitCode 1 for broken relative file link', async () => {
    const file = writeFile('broken-relative.md', '# Title\n\n[link](./missing.md)\n')
    const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {})
    await runLinkValidation([file], baseConfig)
    expect(process.exitCode).toBe(1)
    consoleSpy.mockRestore()
  ...
  test('does not check external links when validateExternalLinks is false', async () => {
    const file = writeFile('external.md', '# Title\n\n[link](https://example.com)\n')
    await runLinkValidation([file], { ...baseConfig, validateExternalLinks: false })
    expect(process.exitCode).toBeUndefined()
  })
  ...
  test('checks external links when validateExternalLinks is true', async () => {
    ...
    const file = writeFile('external.md', '# Title\n\n[link](https://example.com)\n')
    const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {})
    await runLinkValidation([file], { ...baseConfig, validateExternalLinks: true })
    expect(process.exitCode).toBe(1)
    consoleSpy.mockRestore()
  })
  ...
  test('prints warning for 301 redirect', async () => {
    ...
    const file = writeFile('redirect.md', '# Title\n\n[link](https://example.com)\n')
    const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {})
    await runLinkValidation([file], { ...baseConfig, validateExternalLinks: true })
    expect(process.exitCode).toBeUndefined()
    expect(consoleSpy).toHaveBeenCalledWith(
  ...
  test('prints verbose messages when verbose is true', async () => {
    ...
    const file = writeFile('verbose.md', '# Title\n\n[link](https://example.com)\n')
    const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {})
    await runLinkValidation([file], { ...baseConfig, validateExternalLinks: true, verbose: true })
  ...
  test('does not print verbose messages when verbose is false', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({ status: 200 }))
    const file = writeFile('non-verbose.md', '# Title\n\n[link](https://example.com)\n')
    const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {})
    await runLinkValidation([file], { ...baseConfig, validateExternalLinks: true, verbose: false })
