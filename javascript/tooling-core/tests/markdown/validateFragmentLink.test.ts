...
import { validateFragmentLink } from '../../src/markdown/validateFragmentLink.js'
import type { LinkRecord, HeadingRecord } from '../../src/markdown/types.js'
...
describe('validateFragmentLink()', () => {
  ...
  test('returns null for valid fragment', () => {
    const result = validateFragmentLink('README.md', makeLink('#introduction'), headings)
    expect(result).toBeNull()
  })

  test('returns null for valid duplicate-heading slug', () => {
    const result = validateFragmentLink('README.md', makeLink('#install-1'), headings)
    expect(result).toBeNull()
  })

  test('returns error for missing fragment', () => {
    const result = validateFragmentLink('README.md', makeLink('#nonexistent'), headings)
